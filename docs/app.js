(() => {
  ...
  function renderEntryCard(r) {
    ...
    if (hasDiffs) {
      const rows = r.field_diffs.map(d => {
        ...
        const defaultAction = d.suggestedRemove ? "remove" : ((isEnrichment || r.status === "updated") ? "found" : "original");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (xBtn) {
      ...
      if (row.dataset.action === "remove") {
        const r = results[idx];
        const d = r?.field_diffs?.find(diff => diff.field === field);
        const isSuggestedRemove = d && d.suggestedRemove;
        const defaultAction = isSuggestedRemove ? "original" : ((r && r.status === "updated") ? "found" : "original");
        const origVal = decodeURIComponent(row.getAttribute("data-original-val") || "");
    ...
    card.querySelectorAll(".diff-row:not(.field-row-plain)").forEach(row => {
      ...
      if (origPill || sugPill) {
        ...
        if (isAccept) {
          ...
          if (isSuggestedRemove) {
            fieldEdits[idx][field] = { action: "remove", value: "" };
            if (origPill) origPill.classList.remove("active");
            if (sugPill) {
              sugPill.classList.remove("active");
              sugPill.classList.add("removed");
              sugPill.contentEditable = "false";
            }
            row.querySelectorAll(".fa-btn-x").forEach(b => b.classList.add("active"));
            finalAction = "remove";
          } else if (sugPill) {
            const val = sugPill.dataset.val;
            fieldEdits[idx][field] = { action: "found", value: val };
          ...
          row.querySelectorAll(".fa-btn-x").forEach(b => b.classList.remove("active"));
          finalAction = "original";
        }

        syncRowState(row, finalAction);
      } else {
  ...
  function buildPreviewBib() {
    ...
    let final = parsedEntries.slice(0, count).map((entry, i) => {
      ...
      if (s.updateKeys) {
        out.ID = B.generateCitationKey(out);
      }
